...
MCP client -> stdio_proxy.py -> http://127.0.0.1:8765/mcp -> serve.py
```
...
| --- | --- | --- |
| `memory_context` | Compact scoped recall for ordinary agent context | `hint`, `project`, `limit` |
| `memory_context_full` | Larger bootstrap context with pinned and recent items | `project`, `budget` |
| `create_entities` | Add entities and optional initial observations | `entities` |
| `add_observations` | Add observations to existing entities | `observations` |
| `create_relations` | Connect existing entities | `relations` |
| `read_graph` | Read graph data, compressed by default | `tags`, `entity_types`, `limit`, `include_deleted`, `compress` |
| `search_nodes` | FTS5 search with filters | `query`, `tags`, `entity_types`, `time_range`, `limit`, `compress` |
| `open_nodes` | Open named entities and optional neighbors | `names`, `depth` |
| `log_activity` | Record a development or session event | `action`, `summary`, `entity_names`, `tags`, `metadata` |
| `query_timeline` | Query activity history | `time_range`, `start`, `end`, `actions`, `entity_name`, `session_id`, `limit` |
| `manage_tags` | List, create, delete, apply, remove, or clean tags | `action`, `name`, `entity_name`, `tag_name` |
| `merge_entities` | Merge one entity into another | `source`, `target`, `strategy` |
| `export_graph` | Export graph as JSON or JSONL | `format` |
| `import_graph` | Import JSON or JSONL graph data | `data` |
| `memory_stats` | Return counts and storage stats | none |
| `backup_memory` | Copy the active SQLite database | `dest_path` |
| `get_observation_history` | Show observation versions for an entity | `entity_name`, `content_prefix` |
| `delete_entities` | Soft-delete or hard-delete entities | `entityNames`, `hard` |
| `delete_observations` | Delete selected observations | `deletions` |
| `delete_relations` | Delete selected relations | `relations` |
...
| Multiple clients lock the database | Use `serve.py` once and connect clients through `stdio_proxy.py`. |
